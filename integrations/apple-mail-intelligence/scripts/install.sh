...
app_root="${install_root}/FJG Mail Intelligence.app"
...
/bin/mkdir -p "${app_macos}" "${services_root}"
/usr/bin/install -m 0755 "${binary}" "${app_macos}/fjg-mail-intelligence"
/usr/bin/install -m 0644 "${script_directory}/extract-mail.js" "${app_macos}/extract-mail.js"
/usr/bin/install -m 0644 "${integration_root}/AppInfo.plist" "${app_contents}/Info.plist"
...
install_service() {
  ...
  local command_string="/usr/bin/open -W -n -a \"${app_root}\" --args --mode ${mode} --scope prompt"
...
print "Installed on-device Mail Intelligence app: ${app_root}"
